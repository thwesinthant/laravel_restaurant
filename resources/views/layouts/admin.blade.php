...
<body class="font-sans antialiased">
    <div class="flex-col w-full md:flex md:flex-row md:min-h-screen">
        {{-- nav section --}}
        <div @click.away="open = false" class="flex flex-col flex-shrink-0 w-full text-black bg-slate-100 md:w-64 "
            x-data="{ open: false }">
                        ...
                        <div class="px-2 py-2 bg-white rounded-md shadow dark:bg-gray-700">
                            ...
                            <form method="POST" action="{{ route('logout') }}">
                                ...
                                <x-dropdown-link :href="route('logout')"
                                    ...
                                    class="block pe-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg  md:mt-0 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline">
                                    {{ __('Log Out') }}
                                </x-dropdown-link>
                            </form>
                        </div>
        ...
        <main class="m-2 p-8  w-full">
            <div>
                @if (session()->has('danger'))
                    <div class="p-4 mb-4 text-sm text-red-900 rounded-lg bg-red-50 " role="alert">
                        <span class="font-medium">Danger alert!</span> {{ session()->get('danger') }}
                    </div>
                @endif
                @if (session()->has('success'))
                    <div class="p-4 mb-4 text-sm text-green-900 rounded-lg bg-green-50 " role="alert">
                        <span class="font-medium">Success alert!</span> {{ session()->get('success') }}
                    </div>
                @endif
                @if (session()->has('warning'))
                    <div class="p-4 mb-4 text-sm text-yellow-900 rounded-lg bg-yellow-50 d" role="alert">
